...
本系统支持多个专用 Agent，通过 `@Agent名` 触发：

| Agent | 角色 | 触发词 | 说明 |
|-------|------|--------|------|
| CC1 | 开发工程师 | `@CC1` | 代码编写、调试、重构 |
| CC2 | 代码审查员 | `@CC2` | 代码审查、质量检查、最佳实践建议 |
| CC3 | 文档专家 | `@CC3` | 文档编写、README 更新、API 文档 |
...
如果只使用 `@cc` 而不指定具体 Agent，系统会自动分析任务类型并分配给合适的 Agent。

## 超时处理机制

当任务执行超过设定的超时时间（默认 15 分钟）：

- **不会杀死进程**：允许任务继续执行
- **返回进度报告**：包含已收集的输出、执行时间、可能原因和建议
- **保留部分成果**：已产生的 stdout 会被包含在报告中

如果任务频繁超时，建议：
1. 增加 `CC_TIMEOUT` 值
2. 拆分复杂任务为多个子任务
3. 使用 `@CC1` 等明确指定 Agent
